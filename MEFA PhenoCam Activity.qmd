...
Finally, these GCC timeseries can be used to extract seasonal transition dates across the years. For example, to determine the timing of spring green-up, we can used the amplitude (height) of each annual peak to determine when (what day of the year) vegetation at the site has reached either 10%, 25%, or 50% of it's maximum greenness. We consider this the start of spring green-up. We use a similar method to extract the start of autumn brown-down.
...
Over a long time period, the timing of seasonal transition dates can be used to track shifts in vegetation phenology, such as with climate change. This dataset can also be used to understand what environmental and biologic factors (e.g., temperature, rainfall, sunlight) influence the timing of spring leaf-out and/or autumn color changes.

![Seasonal transition dates at a forest site in Canada (lacclair) over the past 9 years.](transition%20dates_timeseries.png){fig-align="center"}
...
Now we'll make an interactive map to view all the PhenoCam sites colored by their primary vegetation type. First, using our "sitelist_table", we filter out any sites for which a primary vegetation type isn't listed or an "NA" is entered. We'll call this new data table "sitelist_table2".
...
Then, let's make the map interactive! You can hover over sites to see their details and draw a box around a specific area to zoom into the region.
...
If you'd like, now try this exercise with another PhenoCam site. Be sure to either clear your working directory folder (delete the downloaded photos) or designate a new folder directory before trying a new site.
